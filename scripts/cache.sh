...
find_wiki_root() {
  ...
  while true; do
    if [ -f "$dir/.wiki-cache.json" ] || [ -f "$dir/.wiki-schema.md" ]; then
      # Convert MSYS path to Windows path (needed on Windows Git Bash)
      # so Python can resolve it correctly
...
# Windows: os.path.relpath raises ValueError across drives; use absolute instead
try:

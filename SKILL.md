...
如果缺失，提示用户查看仓库的 `docs/OPTIONAL_ADAPTERS.md`，分别安装需要的外部适配器。公开仓库不捆绑可选 URL/YouTube 适配器的实现或运行依赖。
...
| "给阅读卡加批注"、"补充我的理解"、"我不同意"、"记录实践"、"更新阅读反馈" | → **reading-card-feedback** |
...
5. 页面格式规则按以下顺序解析：
   - 优先读取 `<wiki_root>/.wiki-rules.md`；这是当前唯一权威来源
   - 仅当根目录文件不存在时，兼容读取 `<wiki_root>/wiki/.wiki-rules.md`
   - 两个文件同时存在时，报告规则冲突并停止写入，等待用户确认；不得自行选择或合并
...
**在生成任何 wiki 页面之前，AI 必须先读取 `<wiki_root>/.wiki-rules.md`。** 该文件是格式的权威来源。只有根目录规则不存在时，才兼容读取 `<wiki_root>/wiki/.wiki-rules.md`。以下为内置快速参考：
...
必填字段：`title`、`date`、`tags`（type/domain/status 各至少一个）、`confidence`。

### 三层中文标签体系

**type（页面性质）— 必选一个**：`type/实体`\|`type/工具`\|`type/概念`\|`type/综述`\|`type/索引`\|`type/日志`

**domain（知识领域）— 至少一个，可多选**：`domain/AI`\|`domain/命令行`\|`domain/开发`\|`domain/Obsidian`\|`domain/效率`\|`domain/法律`\|`domain/财务`\|`domain/通用`

**status（状态）— 必选一个**：`status/草稿`\|`status/已审核`\|`status/过时`\|`status/活跃`

新页面一律 `status/草稿`。index.md 和 log.md 使用 `status/活跃`。

### 领域扩展规则

内容不属于已有 domain 时：AI 提议新标签（2-4 字中文）→ 用户确认 → 写入 `.wiki-rules.md`。
...
创建或更新页面后：
1. 扫描该页面中所有 `[[wikilink]]`
2. 读取每个被引用的已有页面，在其 `## 相关页面` 段追加反向链接
...
- 命中后直接读取，不调用外挂；进入主线的完整文本必须保存在 `raw/`，并记录 `source_path`、`original_ref`、`source_format`、`extraction_method`、`locator_scheme` 和 `source_hash`
- `raw/` 是完整输入文本层，不等于必须把 PDF、DOCX、PPTX、XLSX 或图片等二进制原件复制进 vault；外部原件保留在用户指定位置，通过 `original_ref` 回溯
...
2. **保存完整输入文本**到 `raw/` 对应目录：
   - 根据素材类型保存到对应目录（articles/、tweets/、wechat/、xiaohongshu/、zhihu/ 等）
   - 文件名格式：`{日期}-{短标题}.md`
   - 如果是 URL 类素材，在文件头部记录原始 URL；如果来自 vault 外文件，记录外部原件绝对路径或稳定相对路径
   - `raw/` 内容进入分析后只读。用户批注、反驳和实践反馈只能写入 `wiki/sources/` 阅读卡，不能改写完整输入文本
...
8. **生成素材摘要页**（`wiki/sources/{日期}-{短标题}.md`）：
    ...
    - frontmatter 必须填入 `source_type`、`source_label`、`source_path`、`original_ref`、`source_format`、`extraction_method`、`locator_scheme`、`source_hash` 和 `sources`
    - `sources` 必须指向对应 `raw/` 完整文本；不能留空
    - 阅读卡包含：基本信息、原文定位、核心观点、关键概念、重要摘录、相关页面、我的批注与学习反馈、来源
    - 摘录必须带可回溯定位：PDF 用页码，PPTX 用 slide，XLSX 用工作表和单元格范围，其他文本用标题或引文锚点
...
9. **更新或创建实体页**（`wiki/entities/`）：
   ...
   - 如果没有 → 创建新实体页，**严格按照本文件「页面格式规范」**生成：完整 YAML frontmatter（三层中文标签）、中文段标题（概述/关键事实/相关页面/来源）
   - 使用 `[[实体名]]` 语法做双向链接

10. **更新或创建主题页**（`wiki/topics/`）：
   - 识别素材涉及的主要研究主题
   - 如果已有对应主题页 → 更新素材汇总表和核心观点
   - 如果没有 → 创建新主题页，同样遵循「页面格式规范」

11. **维护双向链接**（必须）：
   - 扫描新创建/更新的页面中所有 `[[wikilink]]`
   - 读取每个被引用的已有页面，检查其 `## 相关页面` 段
   - 若尚未包含反向链接，追加 `- [[新页面]] — 关联说明`
...
3. **生成简化阅读卡**（`wiki/sources/`）：
    - 仍然完整填写阅读卡 frontmatter，`sources` 指向对应 `raw/` 完整文本
    - 保留基本信息、原文定位、核心观点、我的批注与学习反馈和来源
    - 可以省略“重要摘录”，但不得省略“原文定位”
...
## 工作流 2b：reading-card-feedback（阅读卡学习反馈）

当用户说“给阅读卡加批注”“补充我的理解”“我不同意这里的观点”“记录实践”或明确指定一篇已消化素材时使用。用户不需要手动修改 Markdown。

### 前置检查

1. 执行**通用前置检查**，并读取根目录 `.wiki-rules.md`
2. 在 `wiki/sources/` 中查找用户指定的阅读卡；有多个候选时列出标题，请用户确认
3. 读取目标阅读卡、其 `source_path` 指向的完整文本，以及用户本轮明确提供的反馈
4. 不读取或改写 vault 外二进制原件；不改写 `raw/` 完整文本

### 写入规则

1. 若阅读卡缺少新契约字段，先补齐能从现有内容确定的字段；无法确定的历史值使用：
   - `original_ref: "未记录（历史素材）"`
   - `source_format: markdown`
   - `extraction_method: legacy`
   - `locator_scheme: heading_or_quote`
   - `source_hash: legacy-untracked`
2. 若缺少 `我的批注与学习反馈`，在 `相关页面` 后、`来源` 前创建该章节，包含“我的理解”“反驳与保留”“实践与问题”“反馈记录”四个小节
3. 根据用户原话更新对应小节：
   - 用户的解释、联想、记忆线索 → `### 我的理解`
   - 用户不认同、要求核验、提出反例 → `### 反驳与保留`
   - 用户的行动、案例、待研究问题 → `### 实践与问题`
4. 每次都在 `### 反馈记录` 追加 `- {日期}：通过对话更新 {小节名}`
5. 用户意见必须明确标为“我的”内容，不能改写为原文事实，也不能在未获用户指示时改动核心观点、实体页或主题页
6. 如果用户明确要求“基于这条反馈更新知识页”，才更新受影响的 `wiki/entities/`、`wiki/topics/` 或 `wiki/synthesis/` 页面；新增结论标注为 `INFERRED`，并链接回阅读卡
7. 使用 `create-source-page.sh` 写回阅读卡并更新缓存；若没有关联的 `raw/` 文件，则先报告历史素材缺少缓存关联，不伪造缓存命中
